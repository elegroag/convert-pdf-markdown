"""MCP server exposing PDF → Markdown conversion.
...
mcp = FastMCP(
    name="pdf2md",
    instructions=(
        "Convert PDF documents to structured Markdown with headings, "
        "images, tables, links, and code blocks."
    ),
)


def resolve_output_paths(pdf_path: Path, output_path: Path) -> tuple[Path, Path]:
    ...
    slug = AnchorSlug.slugify(pdf_path.stem)
